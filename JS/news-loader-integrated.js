// news.jsonからデータを読み込んでnews.htmlに動的に表示し、ページネーション機能も統合したスクリプト
...
!(function (t, a, e) {
  ...
  (i.prototype = {
    ...
    // news.jsonからデータを読み込んでnews.htmlに動的に表示する処理
    $(document).ready(function() {
        ...
        $.getJSON('Data/news.json', function(news) {
            ...
            // ページネーション設定
            var perPage = 9;
            var totalPages = Math.ceil(news.length / perPage);
            
            // 現在のページを取得
            var currentPage = getCurrentPage(news.length, perPage);
            
            // 表示するページのデータのみを取得
            var startIndex = (currentPage - 1) * perPage;
            var endIndex = startIndex + perPage;
            var currentPageNews = news.slice(startIndex, endIndex);
            
            // 現在のページのニュースデータのみをHTMLに変換
            currentPageNews.forEach(function(newsItem) {
    ...
    function generatePaginationButtons(currentPage, totalPages) {
        ...
        // ページネーションを追加
        $('.latest-news-list').after(paginationHtml);
